# DDD-V0.6.8 — Vision Central Command (La Carte Stratégique)

> **ADR** : ADR-V0.6.8 · **Dossier** : `the-bridge-__-life-os/src/apps/twelve-week/`

---

## Phase A : L'État de Focus (Zustand)

### Étape A.1 : `fw-12wy.store.ts`
Ajouter la gestion de la navigation focale :
```typescript
...
## Phase B : La Carte de Commandement

### Étape B.1 : `VisionCommandCard.tsx`
**NOUVEAU FICHIER** : `src/apps/twelve-week/components/VisionCommandCard.tsx`
...
export function VisionCommandCard({ visionId }: { visionId: string }) {
  ...
  const setActiveGoalId = useTwelveWeekStore(s => s.setActiveGoalId); // From V0.6.9
  
  const vision = useTwelveWeekStore(s => s.visions.find(v => v.id === visionId));
  const childGoals = useTwelveWeekStore(s => s.goals.filter(g => g.visionId === visionId));
  const tactics = useTwelveWeekStore(s => s.tactics);

  const ikigaiRef = useIkigaiStore(s => s.visions.find(v => v.id === vision?.ikigaiVisionId));
  const domainRef = useWheelStore(s => s.nodes.find(v => v.id === vision?.domainId));
  ...
  return (
    <div className="flex-1 flex flex-col p-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="flex items-end justify-between mb-8 pb-8 border-b border-[#FFF]/10">
        <div className="flex gap-4 items-center">
           ...
           <div>
             ...
             <div className="flex items-center gap-3 mt-2">
                ...
                {domainRef && <span className="text-[10px] uppercase font-bold tracking-widest text-white/40">{domainRef.title} Domain</span>}
             </div>
           </div>
        </div>
      ...
      <div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {childGoals.map(goal => {
               const gTactics = tactics.filter(t => t.goalId === goal.id);
               ...
               return (
                  <div 
                    ...
                    className="p-6 rounded-3xl bg-black/40 border border-[#FFF]/10 hover:border-teal-500/40 hover:bg-[#111] transition-all cursor-pointer group shadow-xl"
                  >
                    ...
                    <div className="w-full h-1.5 bg-[#FFF]/5 rounded-full overflow-hidden mt-6">
         ...
         {childGoals.length === 0 && (
           <div className="w-full p-12 border border-dashed border-[#FFF]/10 rounded-3xl flex flex-col items-center justify-center text-center opacity-50">
             <Target className="w-8 h-8 text-[#FFF]/20 mb-4" />
             <p className="text-sm font-medium text-[#FFF]/60">No goals forged for this horizon.</p>
...
```
*(A3: Remplacer le contenu de Dashboard.tsx : Si `activeVisionId` est set, rendre `VisionCommandCard`, sinon le Dashboard normal).*
