# PRD-112: Inter-Domain Conflict Arbitration Matrix
...
- **Write_scope (chemins concrets)** : `src/types/domain-arbitration.ts` (création/mise à jour), `src/services/domain-arbitration-matrix.ts` (création).
...
## Objectif
Résolution systématique des arbitrages inter-domaines : sanctuarisation du couloir Book (LD01) supervisant Saru (LD02) sous veto vital Culber (LD03).

## Spécifications
- Créer `src/types/domain-arbitration.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/domain-arbitration-matrix.ts` avec logique déterministe.
