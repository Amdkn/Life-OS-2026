# PRD-110: 7D Decisional Governance Core & Agent Consensus
...
- **Write_scope (chemins concrets)** : `src/types/governance-core.ts` (création/mise à jour), `src/services/governance-core-engine.ts` (création).
...
## Objectif
Architecture centrale du moteur de consensus déterministe entre essaims d'agents sous arbitrage 7D d'Amadou Kone.

## Spécifications
- Créer `src/types/governance-core.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/governance-core-engine.ts` avec logique déterministe.
