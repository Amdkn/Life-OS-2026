# PRD-111: 12WY Temporal Smart Contracts & Immutable Log
...
- **Write_scope (chemins concrets)** : `src/types/temporal-contract.ts` (création/mise à jour), `src/services/temporal-contract-engine.ts` (création).
...
## Objectif
Spécifier et développer les contrats d'engagement temporel 12WY avec vérification cryptographique des livrables.

## Spécifications
- Créer `src/types/temporal-contract.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/temporal-contract-engine.ts` avec logique déterministe.
