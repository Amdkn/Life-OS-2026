# PRD-113: Sovereign Human Ratification Registry & Cryptographic Signature
...
- **Write_scope (chemins concrets)** : `src/types/ratification-registry.ts` (création/mise à jour), `src/services/ratification-registry-service.ts` (création).
...
## Objectif
Registre inviolable anti-usurpation F5 : verrouillage technique garantissant qu'aucune certification canonique n'est octroyée sans signature explicite d'Amadou Kone.

## Spécifications
- Créer `src/types/ratification-registry.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/ratification-registry-service.ts` avec logique déterministe.
