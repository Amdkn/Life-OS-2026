# PRD-114: Multi-Franchise Decentralized Treasury Dispatcher
...
- **Write_scope (chemins concrets)** : `src/types/treasury-dispatcher.ts` (création/mise à jour), `src/services/treasury-dispatcher-engine.ts` (création).
...
## Objectif
Automatisation de la comptabilité analytique et de la ventilation des flux de trésorerie des 4 franchises B1 (ABC, RILCOT, Alikaly, Marina).

## Spécifications
- Créer `src/types/treasury-dispatcher.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/treasury-dispatcher-engine.ts` avec logique déterministe.
