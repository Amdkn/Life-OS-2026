# PRD-101: Multi-Machine Blackboard Relay Bridge
...
- **Write_scope (chemins concrets)** : `src/types/blackboard-relay.ts` (création/mise à jour), `src/services/blackboard-relay-bridge.ts` (création).
...
## Objectif
Mettre en place la passerelle de communication chiffrée entre blackboards distribués (ex: hôte physique local Kentucky vs instances déportées).

## Spécifications
- Créer `src/types/blackboard-relay.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/blackboard-relay-bridge.ts` avec logique déterministe.
