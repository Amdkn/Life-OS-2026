# PRD-102: Zero-Knowledge PII Vault & Sensitive Data Guard
...
- **Write_scope (chemins concrets)** : `src/types/zk-vault.ts` (création/mise à jour), `src/services/zk-vault-guard.ts` (création).
...
## Objectif
Développer le module de cloisonnement cryptographique Zero-Knowledge pour isoler les secrets, clés API et données personnelles (PII).

## Spécifications
- Créer `src/types/zk-vault.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/zk-vault-guard.ts` avec logique déterministe.
