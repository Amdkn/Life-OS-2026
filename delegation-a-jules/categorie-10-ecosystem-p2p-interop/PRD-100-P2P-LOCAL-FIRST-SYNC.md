# PRD-100: P2P Local-First Sync Engine & CRDT Replication
...
- **Write_scope (chemins concrets)** : `src/types/p2p-sync.ts` (création/mise à jour), `src/services/p2p-sync-engine.ts` (création).
...
## Objectif
Concevoir et implémenter le moteur de synchronisation décentralisé P2P sans dépendance cloud centrale, utilisant des structures de données CRDT (Conflict-Free Replicated Data Types).

## Spécifications
- Créer `src/types/p2p-sync.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/p2p-sync-engine.ts` avec logique déterministe.
