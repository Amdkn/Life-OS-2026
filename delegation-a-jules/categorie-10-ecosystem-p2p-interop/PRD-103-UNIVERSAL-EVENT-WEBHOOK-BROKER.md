# PRD-103: Universal Lightweight Event Webhook Broker
...
- **Write_scope (chemins concrets)** : `src/types/webhook-broker.ts` (création/mise à jour), `src/services/webhook-broker.ts` (création).
...
## Objectif
Créer un courtier d'événements asynchrone append-only ultra-léger (Zero Kafka) basé sur SQLite WAL et triggers HTTP/WS locaux.

## Spécifications
- Créer `src/types/webhook-broker.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/webhook-broker.ts` avec logique déterministe.
