# PRD-105: Network Circuit-Breaker & P2P Health Monitor
...
- **Write_scope (chemins concrets)** : `src/types/circuit-breaker.ts` (création/mise à jour), `src/services/circuit-breaker-monitor.ts` (création).
...
## Objectif
Développer le système d'isolation automatique et coupe-circuit déterministe lors de congestions réseau ou déconnexions.

## Spécifications
- Créer `src/types/circuit-breaker.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/circuit-breaker-monitor.ts` avec logique déterministe.
