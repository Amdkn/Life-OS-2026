# PRD-121: Kardashev Type 2 Civilization Simulator
...
- **Write_scope (chemins concrets)** : `src/types/kardashev-simulator.ts` (création/mise à jour), `src/services/kardashev-simulator-engine.ts` (création).
...
## Objectif
Simulateur dynamique de projection de ressources, calcul, énergie et autonomie pour l'élévation sociétale Solarpunk.

## Spécifications
- Créer `src/types/kardashev-simulator.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/kardashev-simulator-engine.ts` avec logique déterministe.
