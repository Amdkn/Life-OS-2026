# PRD-122: H90 Knowledge Ark & Deep Time Storage Engine
...
- **Write_scope (chemins concrets)** : `src/types/h90-ark.ts` (création/mise à jour), `src/services/h90-ark-storage.ts` (création).
...
## Objectif
Module de sanctuarisation de la mémoire longue (H90 - 90 ans) et transmission intergénérationnelle résiliente.

## Spécifications
- Créer `src/types/h90-ark.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/h90-ark-storage.ts` avec logique déterministe.
