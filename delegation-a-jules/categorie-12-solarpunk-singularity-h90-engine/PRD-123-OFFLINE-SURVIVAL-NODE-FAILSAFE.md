# PRD-123: Offline Survival Node & Disaster Recovery Failsafe
...
- **Write_scope (chemins concrets)** : `src/types/survival-node.ts` (création/mise à jour), `src/services/survival-node-failsafe.ts` (création).
...
## Objectif
Protocole de survie en isolement total réseau : fonctionnement hermétique avec persistance locale intégrale et reprise sans perte.

## Spécifications
- Créer `src/types/survival-node.ts` avec les interfaces et types TypeScript fondamentaux.
- Implémenter le service ou composant dans `src/services/survival-node-failsafe.ts` avec logique déterministe.
