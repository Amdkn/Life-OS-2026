  ...
  return (
    <div className="mt-4 p-6 rounded-2xl bg-black/40 border border-white/10 animate-in slide-in-from-top-2 duration-300">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-sm font-bold uppercase tracking-widest text-[var(--theme-text)]" style={{ color }}>
          {domain} › {pillar}
        </h3>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5 text-center">
          <span className="text-2xl font-black text-[var(--theme-text)]/60">{healthGauge}%</span>
      ...
      <div className="space-y-2 mb-6">
        ...
        {projects.length === 0 ? (
          <p className="text-[10px] text-white/15 italic py-2 text-[var(--theme-text)]/40">No active projects for this pillar.</p>
        ) : projects.map(p => (
          <div key={p.id} className="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex justify-between text-xs">
            <span className="text-[var(--theme-text)]/60">{p.title}</span>
            <span className="text-[var(--theme-text)]/20">{p.progress}%</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2 mb-6">
        ...
        {gtdActions.length === 0 ? (
          <p className="text-[10px] text-white/15 italic py-2 text-[var(--theme-text)]/40">No actionable items for this domain.</p>
        ) : gtdActions.slice(0, 5).map(a => (
          <div key={a.id} className="p-3 rounded-xl bg-white/[0.02] border border-white/5 text-xs text-[var(--theme-text)]/50">
            {a.content}
          </div>
        ))}
      </div>
