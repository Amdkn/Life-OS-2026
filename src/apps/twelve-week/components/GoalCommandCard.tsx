...
export function GoalCommandCard({ goalId }: { goalId: string }) {
  ...
  const tactics = allTactics.filter(t => t.goalId === goalId);
           ...
           <div className="p-6 rounded-3xl bg-blue-500/5 border border-blue-500/10">
              ...
              {linkedProject ? (
                 <div className="p-4 bg-black/60 rounded-xl border border-white/5">
                   ...
                   <p className="text-[9px] uppercase tracking-widest text-white/40 mt-1">Project Active</p>
                 </div>
              ) : (
                 <div className="p-4 bg-black/40 rounded-xl border border-dashed border-white/10 text-center">
                   <p className="text-[10px] uppercase tracking-widest text-white/30">Unlinked Execution</p>
                 </div>
              )}
           </div>

           {/* GTD INBOX PLACEHOLDER */}
           <div className="p-6 rounded-3xl bg-[#111]/80 border border-dashed border-white/10">
